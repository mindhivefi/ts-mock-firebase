 ...
 */
export function validateExactNumberOfArgs(functionName: string, args: IArguments, numberOfArgs: number): void {
 ...
 */
export function validateAtLeastNumberOfArgs(functionName: string, args: IArguments, minNumberOfArgs: number): void {
 ...
 * using typeof checks.
 */
export function validateArgType(functionName: string, type: ValidationType, position: number, argument: unknown): void {
...
function validateType(functionName: string, type: ValidationType, inputName: string, input: unknown): void {
  let valid = false;
  if (type === 'object') {
  ...
  if (!valid) {
    ...
    throw new FirestoreError(
      Code.INVALID_ARGUMENT,
      `Function ${functionName}() requires its ${inputName} ` + `to be of type ${type}, but it was: ${description}`
    );
  }
}
...
export function validateDefined(functionName: string, position: number, argument: unknown): void {
  if (argument === undefined) {
    throw new FirestoreError(
      Code.INVALID_ARGUMENT,
      `Function ${functionName}() requires a valid ${ordinal(position)} ` + `argument, but it was undefined.`
    );
  }
}
 ...
 */
export function validateOptionNames(functionName: string, options: object, optionNames: string[]): void {
 ...
 * an instanceof check.
 */
export function invalidClassError(functionName: string, type: string, position: number, argument: unknown): Error {
