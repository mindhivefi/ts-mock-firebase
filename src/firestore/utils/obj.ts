...
export function forEachNumber<V>(obj: Dict<V>, fn: (key: number, val: V) => void): void {
  for (const key in obj) {
    if (Object.prototype.hasOwnProperty.call(obj, key)) {
...
export function forEach<V>(obj: Dict<V>, fn: (key: string, val: V) => void): void {
  for (const key in obj) {
    if (Object.prototype.hasOwnProperty.call(obj, key)) {
      fn(key, obj[key]);
    }
...
export function lookupOrInsert<V>(obj: Dict<V>, key: string | number, valFn: () => V): V {
  if (!contains(obj, key)) {
    obj[key] = valFn();
  }
  return obj[key];
